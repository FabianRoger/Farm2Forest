...
F2F_regions <- st_read("Shapes/prod_län_combi_byyann_skarning2.shp", stringsAsFactors = FALSE, options = "ENCODING=latin1")
...
RDP_cl <- 
RDP_cl %>%
  mutate(in_study_region = map(geometry, function(x) TRUE %in% st_within(x, F2F_regions$geometry, sparse = F))) %>% 
   collect() %>% 
  unnest(in_study_region)

time_elapsed_parallel <- proc.time() - start 

RDP_cl <- st_as_sf(RDP_cl)
st_crs(RDP_cl) <- st_crs(RPD_2012)
...
```{r}
Kundr_in_studyRegion <- filter(RDP_cl, in_study_region) %>% 
...
+ check what proportion of kundnr have fields ouside the study regions
```{r}
...
For those with blocks both inside and outside the study region, what proportion (by area) is inside teh study region?
...
`%` of customer number from 2012 Livestock database also present in RPD
...
`%` of customer number RDP in Livestock database
...
RPD_2012$kundnr1 %>%
  ...
  sum() %>% 
  `/`(length(unique(RPD_2012$kundnr1))) %>% 
  `*`(100)
```
...
+ join livestock data to the blockdatabase 
  + by `knudnr1` ignoring the others
  
+ join crop codes to the blockdatabase

```{r, eval = TRUE}
RPD_2012 <- left_join(RPD_2012, Livestock)

RPD_2012 <- left_join(RPD_2012, crop_codes)
